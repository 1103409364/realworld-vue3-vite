...
import {
  deleteFollowProfile,
  postFollowProfile,
} from "src/services/profile/followProfile";
...
describe("# ArticleDetailMeta", () => {
  ...
  >;
  const mockFollowUser = postFollowProfile as jest.MockedFunction<
    typeof postFollowProfile
  >;
  const mockUnfollowUser = deleteFollowProfile as jest.MockedFunction<
    typeof deleteFollowProfile
  >;
  const mockFavoriteArticle = postFavoriteArticle as jest.MockedFunction<
    typeof postFavoriteArticle
  >;
  const mockUnfavoriteArticle = deleteFavoriteArticle as jest.MockedFunction<
    typeof deleteFavoriteArticle
  >;

  beforeEach(async () => {
    mockFollowUser.mockResolvedValue({ isOk: () => true } as any);
    mockUnfollowUser.mockResolvedValue({ isOk: () => true } as any);
    mockFavoriteArticle.mockResolvedValue({
      isOk: () => true,
      value: fixtures.article,
    } as any);
    mockUnfavoriteArticle.mockResolvedValue({
      isOk: () => true,
      value: fixtures.article,
    } as any);
  ...
  it("should display edit button when user is author", () => {
    ...
    expect(
      queryByRole("button", { name: followButton })
  ...
  it("should call unfollow service when click follow button and not followed author", async () => {
    ...
    const { getByRole } = render(ArticleDetailMeta, {
      ...
      props: {
        article: {
          ...fixtures.article,
          author: { ...fixtures.article.author, following: true },
        },
      },
    });
