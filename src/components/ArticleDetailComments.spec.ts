...
describe("# ArticleDetailComments", () => {
  ...
  beforeEach(() => {
    cy.intercept("GET", "/api/profiles/*", { profile: fixtures.author }).as(
      "getProfile"
    );
    cy.intercept("GET", "/api/articles/*/comments", {
      comments: [fixtures.comment],
    }).as("getCommentsByArticle");
    cy.intercept("POST", "/api/articles/*/comments", {
      comment: fixtures.comment2,
    }).as("postCommentsByArticle");
    cy.wrap(
      router.push({ name: "article", params: { slug: fixtures.article.slug } })
    );
  });
  ...
  it("should display new comment when post new comment", () => {
    ...
    // when
    cy.findByRole("textbox", { name: "Write comment" }).type(
      fixtures.comment2.body
    );
  ...
  it.only("should call remove comment service when click delete button", () => {
    // given
    cy.intercept("DELETE", "/api/articles/*/comments/*", { status: 200 }).as(
      "deleteComment"
    );
    userStore.updateUser(fixtures.user);
    cy.mount(AsyncArticleDetailComments, { router });
