...
export type PostRegisterErrors = Partial<
  Record<keyof PostRegisterForm, string[]>
>;

export async function postRegister(
  form: PostRegisterForm
): Promise<Either<ValidationError<PostRegisterErrors>, User>> {
  const result1 = await request.checkablePost<UserResponse>("/users", {
    user: form,
  });
  const result2 = mapValidationResponse<PostRegisterErrors, UserResponse>(
    result1
  );
