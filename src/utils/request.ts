...
export default class FetchRequest {
  ...
  private readonly handleResponse = <T>(
    response: Response
  ...
  get<T = unknown>(
    ...
    return this.runSafeFetch("GET", url, options).then((r) =>
      this.handleCorrectResponse<T>(r)
    );
  }

  checkableGet<T = unknown>(
    url: string,
    options?: Partial<FetchRequestOptions>
  ): Promise<Either<NetworkError, T>> {
    return this.runSafeFetch("GET", url, options).then((r) =>
  ...
  post<T = unknown>(
    ...
    return this.runUnsafeFetch("POST", url, data, options).then((r) =>
      this.handleCorrectResponse<T>(r)
    );
  }

  checkablePost<T = unknown>(
    url: string,
    data?: unknown,
    options?: Partial<FetchRequestOptions>
  ): Promise<Either<NetworkError, T>> {
  ...
  delete<T = unknown>(
    ...
    return this.runSafeFetch("DELETE", url, options).then((r) =>
      this.handleCorrectResponse<T>(r)
    );
  }

  checkableDelete<T = unknown>(
    url: string,
    options?: Partial<FetchRequestOptions>
  ): Promise<Either<NetworkError, T>> {
    return this.runSafeFetch("DELETE", url, options).then((r) =>
  ...
  put<T = unknown>(
    ...
    return this.runUnsafeFetch("PUT", url, data, options).then((r) =>
      this.handleCorrectResponse<T>(r)
    );
  }
  ...
  patch<T = unknown>(
    ...
    return this.runUnsafeFetch("PATCH", url, data, options).then((r) =>
      this.handleCorrectResponse<T>(r)
    );
  }
