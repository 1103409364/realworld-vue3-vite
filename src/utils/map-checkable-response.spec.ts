...
} from "./map-checkable-response";

const createCheckableResponse = (
  response: Partial<Response>
): Either<NetworkError, Partial<Response>> =>
  response.ok === true
    ? success(response)
...
describe("# mapValidationResponse", function () {
  ...
  it("should return Either with ValidationError and correct Response", function () {
    ...
    const result = mapValidationResponse<ValidationErrors, Partial<Response>>(
      response
    );
  ...
  wrapTests({
    ...
    fn: async (status) => {
      const RESPONSE = {
        ok: false,
        status,
        json: () => Promise.resolve({ errors: { foo: "bar" } }),
      };
      const response = createCheckableResponse(RESPONSE);
      ...
      expect(result.isFail() && (await result.value.getErrors())).toEqual(
