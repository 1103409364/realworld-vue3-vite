...
function isCheckableSafe(
  method: CheckableMethod
): method is CheckableSafeMethod {
  return ["checkableGet", "checkableDelete"].includes(method);
}
function isCheckable(
  method: CheckableMethod | Method
): method is CheckableMethod {
  return [
    "checkableGet",
    "checkableDelete",
    "checkablePost",
...
async function triggerMethod<T = unknown>(
  request: FetchRequest,
  method: Method | CheckableMethod,
  options?: Partial<FetchRequestOptions>
): Promise<T | Either<NetworkError, T>> {
  if (isCheckable(method)) {
    let response: Either<NetworkError, T>;
    if (isCheckableSafe(method))
      response = await request[method]<T>(PATH, options);
...
function forCorrectMethods(
  task: string,
  fn: (method: Method) => Promise<void>
): void {
  wrapTests<Method>({
    task,
    fn,
...
function forCheckableMethods(
  task: string,
  fn: (method: CheckableMethod) => Promise<void>
): void {
...
function forAllMethods(
  task: string,
  fn: (method: Method | CheckableMethod) => Promise<void>
): void {
...
describe("# Should convert query object to query string in request url", () => {
  ...
  forAllMethods("should implement global + local query", async (method) => {
    ...
    expect(global.fetch).toBeCalledWith(
      `${PATH}?${params2query(expectedOptions.params)}`,
      expect.any(Object)
    );
  });
});
...
forCorrectMethods(
  "# Should converted correct response body to json",
...
forCheckableMethods(
  "# Should converted checkable response to Either<NetworkError, DATA_TYPE>",
  async function (method) {
    const DATA = { foo: "bar" };
    interface DATA_TYPE {
      foo: "bar";
    ...
    const resultValue =
      isEither<unknown, DATA_TYPE>(result) && result.isOk()
        ? result.value
        : null;
...
forCorrectMethods(
  "# Should throw NetworkError if correct request is not OK",
  async function (method) {
    mockFetch({
      type: "full",
      ok: false,
...
forCheckableMethods(
  "# Should return Either<NetworkError, DATA_TYPE> if checkable request is not OK",
  async function (method) {
    mockFetch({
      type: "full",
      ok: false,
    ...
    const resultIsNotOk =
      isEither<NetworkError, unknown>(result) && result.isFail();
    const resultValue =
      isEither<NetworkError, unknown>(result) && result.isFail()
        ? result.value
        : null;
